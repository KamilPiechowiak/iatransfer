...
def get_model_timm(classes, channels, name="dla46x_c"):
    ...
    model = timm.create_model(
        name, num_classes=classes, in_chans=channels, pretrained=False
    )
    model.name = name
    return model
...
def get_cifar_models():
    ...
    json = read_json(PATH)["models"]
    models = []
    for t_json in json[:8]:
        t = TrainingTuple.from_json(t_json)
        m = t.model()
        m.name = t_json["model"]["name"]
        models.append(m)
    return models
...
def eval_tls_score(debug):
    ...
    elif SMALL_RESNETS is not None:
        _models = get_cifar_models()
    else:
        _models = [
    ...
    for model_to_name, _model_to in pbar:
        ...
        for model_from_name, model_from in tqdm(models.items(), position=0, leave=True):
            score = iat.sim(model_from, model_to)
...
def experiment_3(debug=False, path=None):
    if not path:
        ...
        df.to_csv(f"figures/matrix{suffix}.csv", index=True)
    else:
        df = pd.read_csv(path)
        df = df.set_index("name")
...
if __name__ == "__main__":
    # PATH = "config/sim/models.csv"
    # res = get_list_of_models()
    # print(len(res))
    # print(res)
    # experiment_3(path="figures/matrix.csv")
    PATH = "config/models/small.json"
    SMALL_RESNETS = True
    # print(get_cifar_models())
    experiment_3()
